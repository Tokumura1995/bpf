...
char keys[][4] = {"abcd", "efgh", "ijkl", "mnop", "qrst", "uvwx"};
...
static inline __u64 cache_check(void * key)
{
  int i = 0;
  printf("a\n");
  while (i < 4) {
    if ((strcmp((char *) key, keys[i])) == 0) {
      return (__u64) (unsigned long)1;
    } else {
      return (__u64) (unsigned long)0;
    }
  }
}
...
int main(int argc, char ** argv)
{
  ...
  */
  ...
  struct bpf_insn prog[] = {
    ...
    BPF_LD_ABS(BPF_W, 8),
    BPF_JMP_IMM(BPF_JEQ, cache_check(BPF_REG_0), 0, 10),
